...
Saya menggunakan Claude Code (Model Fable 5 dan Sonnet 5) untuk *brainstorming* materi, mencari referensi, dan melakukan *troubleshooting* kode `quarto`. Saya bertanggung jawab penuh atas substansi dan penyajian presentasi ini.
...
::: {.incremental}
1. Menggunakan ChatGPT/Claude/Gemini/Copilot/Kimi/DeepSeek minimal sekali
2. Menggunakannya untuk tugas kuliah atau pekerjaan
3. Menggunakannya untuk urusan **di luar** kuliah dan pekerjaan (e.g., minta resep, curhat, menyusun rencana liburan, dsb.)
4. Pernah **mendapati** AI memberikan jawaban yang salah, tapi terdengar sangat meyakinkan
:::

::: {.callout-note}
### Ingat jawaban Anda tadi
...
## *Artificial intelligence* dan "akal imitasi"
...
* Padahal yang sebenarnya terjadi: ia adalam model statistik dengan *output* teks ("*large langauge model*") yang **meniru** produk kognisi manusia ("bahasa natural")
* Frasa *akal imitasi* mengingatkan kita pada dua hal sekaligus:
  + Kemampuan *large language model* (LLM) secara aktual memang **sangat berguna** karena teks *output*nya memang sangat meyakinkan
  + Tapi ia **bukan manusia**, sehingga tidak punya pemahaman, niat, apalagi tanggung jawab
* Konsekuensinya: yang bertanggung jawab atas hasil kerjanya adalah **kita** sebagai penggunanya
:::
...
* **2010-an** — *deep learning*: jaringan saraf tiruan berlapis-lapis; terobosan besar dalam pengenalan gambar dan suara (*neuro-linguistic programming*, NLP)
...
*Large language model* (LLM) pada intinya adalah model statistik raksasa yang menjawab satu pertanyaan:

> **"Dari semua *input* kata sejauh konteks percapakan ini, kata apa yang paling mungkin muncul berikutnya?"**
...
Terobosan kuncinya adalah mekanisme ***attention***: setiap kata bisa "memperhatikan" konteks dari semua kata lain dalam konteks untuk menentukan maknanya.

::: {.incremental}
* "Bisa **ular** itu berbahaya" vs. "Saya **bisa** datang besok"
  + "Bisa" adalah kata yang sama, makna berbeda; *attention* menangkap perbedaannya dari konteks
...
2. ***Fine-tuning*** (termasuk RLHF, *reinforcement learning from human feedback*) — model dilatih mengikuti instruksi, menjawab dengan sopan, dan menolak permintaan tidak patut bahkan berbahaya
   + Hasilnya: *Chatbot* seperti ChatGPT, DeepSeek, atau Claude yang Anda kenal sekarang
:::

::: {.callout-tip}
### Analogi psikologinya 
...
Lanskapnya bergerak sangat cepat:

::: {.incremental}
* ***Chatbot*** — merespons pesan Anda satu per satu (ChatGPT-3.5, 2022)
* ***Reasoning models*** — "berpikir" langkah demi langkah dulu sebelum menjawab (e.g., model-model "*frontier*" seperti Opus, Kimi, Fable, etc.)
* ***AI agents*** — merencanakan dan **mengeksekusi** tugas multi-langkah secara mandiri atas nama penggunanya: menjelajah web, memakai aplikasi, menulis dan menjalankan kode pemrograman [Yang et al. (2025)](https://arxiv.org/abs/2512.07828)
...
::: {.incremental}
* **Halusinasi** — menghasilkan informasi salah dengan *tone* yang sangat meyakinkan, termasuk **mengarang referensi ilmiah** 
...
* ***Context window*** terbatas — *working memory*-nya bisa penuh; percakapan dan dkumen yang sangat panjang terpaksa terpotong, tidak selesai diproses
* **Penalaran yang rapuh** — LLM bisa memberikan jawaban yang sama sekali berbeda ketika *prompt*nya diubah yang sedikit saja dari pola *data training*nya 
...
::: {.callout-important}
### *Human-AI collaboration* menjadi keterampilan esensial

Artinya: kemampuan berkolaborasi dengan AI, ke depan, akan menjadi keterampilan dasar, seperti halnya literasi digital.
:::
...
::: {.incremental}
* Penggunaan **non-pekerjaan** tumbuh lebih cepat daripada penggunaan AI untuk perkerjaan: dari 53% (2024) menjadi **>70%** (2025) dari seluruh pesan
...
* *Value* utamanya adalah ***decision support***: AI sebagai teman berpikir untuk pekerjaan produksi pengetahuan
:::
...
::: {.fragment}
**Dua pesan untuk Anda** sebagai mahasiswa magister psikologi:

1. AI sudah menjadi bagian dari infrastruktur aktivitas akademik. Fokusnya saat ini adalah belajar *bagaimana* caranya menggunakan AI
2. Oleh karena itu, menggunakan AI dengan **efektif, efisien, etis, dan aman** harus dipelajari dan diusahakan dengan sungguh-sungguh
3. Menggunakan AI harus dipelajari mulai sekarang, **sebelum menggunakan AI semakin kompleks dan mahal**
:::
...
[Rogers & Carbonaro (2025)](https://jtl.uwindsor.ca/index.php/jtl/article/view/9420) membedakan dua jenjang:
...
::: {.fragment}
Analoginya seperti belajar bahasa: *literacy* = bisa membaca; *fluency* = bisa bercakap-cakap dengan lancar. Bagian 1–2 tadi adalah *literacy*; sisa sesi ini tentang *fluency*.
:::
...
* Pengguna fasih bersikap ***augmentative*** — bolak-balik menyempurnakan, mempertajam tujuan, mengkritisi *output*
* Pengguna pemula bersikap **pasif** — menerima begitu saja rencana dan jawaban AI
:::

::: {.fragment}
**Paradoksnya:** pengguna fasih justru mengalami **lebih banyak kegagalan** (i.e., menolak *output* AI), tapi kegagalan mereka *visible*, karena mereka aktif memeriksa, dan lebih sering berujung perbaikan *output*. 

Kegagalan pemula justru *invisible*: percakapan tampak sukses men*deliver* *output*, padahal hasilnya meleset dan tidak koheren sama sekali.
:::
...
* ***Agency*** — Anda mengonfigurasi AI untuk bekerja mandiri atas nama Anda
  + *"Setiap ada artikel baru tentang topik X, ringkas dan kirimkan ke saya"*
:::

::: {.fragment}
Riset menunjukkan mode yang paling produktif untuk pekerja pengetahuan adalah ***augmentation***, yaitu AI sebagai *thought partner*, bukan pengganti ([Potts & Sudhof, 2026](http://arxiv.org/abs/2604.25905)).
:::
...
## *Delegation* dalam satu slide

Sebelum meminta *chatbot* melakukan sesuatu, tanyakan 3️⃣ hal ini pada diri Anda sendiri:

::: {.incremental}
1. ***Problem awareness*** — apa sebenarnya tujuan saya? Seperti apa hasil yang baik?
2. ***Platform awareness*** — AI (yang mana) bisa apa, dan tidak bisa apa?
...
Kualitas *output* AI sangat ditentukan oleh kualitas komunikasi Anda. Tiga komponen *description*:
...
::: {.callout-caution}
### Perlakukan AI sebagai kolaborator 

Perlakukan AI sebagai kolaborator, bukan "jin botol"🧞. Komunikasikan dan evaluasi *output* AI, meskipun Anda menolak *output* dari percakapan tersebut. 
:::

:::
::: {.column width="40%"}

![](libs/jin.png){fig-align="center" width="85%"}
...
::: {.fragment}
Keduanya lalu membentuk *feedback loop*: *describe* → *discern* → perbaiki deskripsi → *discern* lagi... sampai hasilnya benar-benar layak. 
:::
...
::: {.incremental}
* Kegagalan pengguna pemula bersifat ***invisible*** — percakapan tampak sukses, penggunanya puas, padahal hasilnya **tidak koheren sama sekali karena tidak pernah diperiksa secara kritis**
...
* Pengguna yang fasih lebih sering *menemukan* kegagalan justru karena mereka **aktif mencarinya**
:::
...
* **Efektif** — kalau Anda cukup ahli untuk *mengarahkan* AI
* **Efisien** — kalau Anda cukup ahli untuk *memverifikasi dengan cepat* *output* teks AI
* **Etis** — kalau Anda cukup ahli untuk *mempertanggungjawabkan* *output* teks AI
* **Aman** — kalau Anda cukup ahli untuk *menangkap kesalahan dalam output AI sebelum merugikan diri Anda sendiri dan orang lain*
...
::: notes
Ini pesan sentral kuliah ini — beri waktu. Hubungkan ke pengalaman mereka: "pernahkah menyerahkan tugas berbantuan AI yang sebenarnya tidak Anda pahami? Bagaimana rasanya kalau ditanya pembimbing?" Tegaskan: menjadi psikolog/peneliti yang lebih ahli adalah strategi AI terbaik.
:::
...
* ***Deployment diligence*** — mengambil tanggung jawab penuh atas *output* yang Anda gunakan dan sebarkan
  + Anda yang menandatangani, Anda yang menjamin isinya
:::
...
* Periksa: apakah AI *system* yang Anda pakai menggunakan *input* Anda untuk melatih modelnya?
* Pikirkan dua kali sebelum mengunggah dokumen-dokumen ini ke AI *system*: naskah yang belum terbit, ide penelitian yang belum dibuat publik, dokumen internal institusi
* Pilihan yang lebih aman selalu ada: anonimkan dulu, pakai data sintetis untuk contoh, atau pilih AI *system* dengan jaminan privasi yang jelas
:::
...
* Kalau ragu, prinsipnya sederhana: **lebih baik terbuka sejak awal**
:::
...
* ***Description*** — komunikasi yang jernih menjembatani maksud Anda dan kemampuan AI
* ***Discernment*** — evaluasi kritis menjaga kualitas; carilah potensi kegagalan sebelum ia "menggagalkan" Anda
* ***Diligence*** — akuntabilitas dan transparansi menjaga integritas Anda
:::

::: {.fragment}
🗝️nya: keterlibatan aktif dalam *human-AI collaboration*, bukan penerimaan pasif. Ini yang membedakan AI sebagai *asisten* dari AI sebagai "jin botol"🧞 ([Potts & Sudhof, 2026](http://arxiv.org/abs/2604.25905)).
:::

::: {.fragment}
**Satu *skill*** yang membuat Anda tidak tergantikan oleh AI: *domain expertise* Anda. Menjadi psikolog dan peneliti yang semakin ahli adalah strategi *human-AI collaboration* yang terbaik yang bisa Anda terapkan. 

Ingat, *fluency* tidak datang dari satu kuliah matrikulasi😄, ia tumbuh lewat praktik yang dilakukan berulang-ulang secara reflektif.
:::

## 🧩 Aktivitas: Uji AI dengan keahlian Anda
...
* Chatterji, A., Cunningham, T., Deming, D., Hitzig, Z., Ong, C., Shan, C., & Wadman, K. (2025). *How people use ChatGPT*. OpenAI/NBER Working Paper.
* Fung, P., Bachrach, Y., Celikyilmaz, A., et al. (2025). *Embodied AI agents: Modeling the world* (arXiv:2506.22355). arXiv. <https://arxiv.org/abs/2506.22355>
* Ghali, M.-K., Farrag, A., Sakai, H., El Baz, H., Jin, Y., & Lam, S. (2024). *GAMedX: Generative AI-based medical entity data extractor using large language models* (arXiv:2405.20585). arXiv. <https://arxiv.org/abs/2405.20585>
...
* Russell-Lasalandra, L. L., Christensen, A. P., & Golino, H. (2024). *Generative psychometrics via AI-GENIE: Automatic item generation and validation with network-integrated evaluation* [preprint]. PsyArXiv. <https://doi.org/10.31234/osf.io/fgbj4>
