---
title: "Teknologi Pembelajaran untuk Kebutuhan Khusus"
subtitle: "MK Teknologi dalam Pembelajaran"
author: "Rizqy Amelia Zein"
institute:
  - "[_Departemen Psikologi, Universitas Airlangga_](https://psikologi.unair.ac.id/dosen-rizqy-amelia-zein/)"
date: today
format:
  unair-revealjs:
    slide-number: true
    progress: true
    transition: slide
    center: false
    chalkboard: true
execute:
  eval: false
  echo: true
---

## Agenda {.agenda-slide}

1. **Mengapa aksesibilitas penting?**
2. **Ragam kebutuhan khusus** dan hambatan belajar yang menyertainya
3. ***Universal Design for Learning* (UDL)**
4. **Teknologi pembantu** (*assistive technology*) per kategori kebutuhan
5. **Peran akal imitasi** dalam memperluas aksesibilitas
6. **Prinsip desain aksesibel** (WCAG) dan isu etis yang menyertainya

## Mengapa aksesibilitas penting?

<div style="display: flex; justify-content: center;">
<iframe width="800" height="450" src="https://www.youtube.com/embed/KmFPWxjmnqE?si=2pqHOnq4Bc_OfqyM" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
</div>

# Mengapa Aksesibilitas Penting? ⚖️ {background-color="#14497F"}

## Aksesibilitas adalah hak

::: {.incremental}
* Aksesibilitas dalam pendidikan adalah **hak** peserta didik yang harus dipenuhi.
* Indonesia meratifikasi *Convention on the Rights of Persons with Disabilities* (CRPD) PBB melalui **UU No. 19 Tahun 2011**.
* Diperkuat oleh [**UU No. 8 Tahun 2016 tentang Penyandang Disabilitas**](https://peraturan.bpk.go.id/Details/37251/uu-no-8-), yang menjamin hak atas pendidikan inklusif di semua jenjang.
:::

## Model disabilitas: Model medis vs. sosial

::: {.columns}
::: {.column width="50%"}
**Model medis** (lama)

::: {.incremental}
* Disabilitas = "kekurangan" pada individu
* Solusi = "perbaiki" atau isolasi individu tersebut
* Fokus pada diagnosis dan keterbatasan
:::
:::

::: {.column width="50%"}
**Model sosial** (modern)

::: {.incremental}
* Disabilitas muncul dari **ketidaksesuaian** antara individu dan lingkungan
* Solusi = ubah **lingkungan** (termasuk desain teknologi dan pembelajaran)
* Fokus pada penghapusan hambatan (*barrier removal*)
:::
:::
:::

::: {.callout-note}
### Implikasi bagi pendidik
Kalau seorang peserta didik "gagal" mengakses materi maka perlu direfleksikan **"apa yang salah dengan desain materinya?"**
:::

# Ragam Kebutuhan Khusus 🧩 {background-color="#14497F"}

## Empat kategori hambatan belajar yang umum

::: {.incremental}
1. **Sensorik** — hambatan penglihatan (*low vision*, *blind*) dan pendengaran (*hard of hearing*, *deaf*)
2. **Motorik/fisik** — keterbatasan gerak yang memengaruhi penggunaan perangkat standar (*mouse*, *keyboard*)
3. **Kognitif dan belajar spesifik** — disleksia, diskalkulia, ADHD, gangguan spektrum autisme
4. **Kesehatan mental dan kondisi kronis** — kecemasan, depresi, kelelahan kronis yang memengaruhi kapasitas atensi dan energi belajar
:::

::: {.callout-tip}
### Ingat
Hambatan belajar juga bisa **situasional** atau **sementara** (koneksi internet buruk, tangan cedera, lingkungan bising), sehingga desain yang aksesibel menguntungkan **semua orang**, bukan hanya penyandang disabilitas permanen ([*"curb-cut effect"*](https://en.wikipedia.org/wiki/Curb_cut_effect)).
:::

# Universal Design for Learning 🎨 {background-color="#14497F"}

## Merancang untuk keberagaman sejak awal

Kerangka **UDL** dari [CAST](https://udlguidelines.cast.org/) menawarkan tiga prinsip utama:

::: {.incremental}
1. ***Multiple means of representation*** — sajikan informasi lewat lebih dari satu format (teks, audio, visual, video bertakarir)
2. ***Multiple means of action & expression*** — beri peserta didik lebih dari satu cara menunjukkan pemahaman (esai, rekaman suara, infografis, presentasi)
3. ***Multiple means of engagement*** — beri pilihan dan relevansi agar motivasi belajar tetap terjaga
:::

::: aside
[CAST (2024). *UDL Guidelines* versi 2.2](https://udlguidelines.cast.org/)
:::

## UDL dirancang untuk semua peserta didik

::: {.callout-important}
### Prinsip kunci
UDL dirancang **untuk semua peserta didik di kelas**, termasuk penyandang disabilitas, sejalan dengan *curb-cut effect* yang sudah kita bahas
:::

Bandingkan dengan pendekatan lama: akomodasi *ad hoc* yang baru diberikan **setelah** peserta didik melapor kesulitan. Respons semacam ini reaktif dan sering terlambat.

# Teknologi Pembantu (Assistive Technology) 🛠️ {background-color="#14497F"}

## Definisi dan cakupannya

> **"*Assistive technology* adalah produk, peralatan, atau sistem apa pun yang meningkatkan, memelihara, atau memperbaiki kapabilitas fungsional individu penyandang disabilitas."** — *Assistive Technology Act* (AS)

::: {.incremental}
* Rentangnya luas: dari **teknologi rendah** (kartu bergambar, pengatur waktu visual) hingga **teknologi tinggi** (*eye-tracker*, *brain-computer interface*)
* Tidak harus canggih/kompleks; yang penting **tepat guna** sesuai kebutuhan individu
:::

## Untuk hambatan penglihatan

::: {.incremental}
* ***Screen reader*** — membacakan teks di layar (NVDA, JAWS, VoiceOver)
* ***Text-to-speech*** (TTS) untuk bahan bacaan digital
* *Braille display* elektronik dan alat cetak braille
* Perbesaran layar (*screen magnification*) dan mode kontras tinggi
:::

## Untuk hambatan pendengaran

::: {.incremental}
* Teks berjalan/*closed captioning* otomatis (kini banyak didukung *live transcription* berbasis AI)
* *Hearing loop* dan alat bantu dengar dengan konektivitas *bluetooth*
* Notifikasi visual/getar sebagai pengganti alarm suara
* Interpretasi bahasa isyarat (manusia atau avatar berbasis AI, meski masih terbatas akurasinya)
:::

## Untuk hambatan motorik

::: {.incremental}
* Papan ketik dan *mouse* adaptif (*switch access*, *joystick*, *eye-gaze*)
* Kontrol suara (*voice control/dictation*) untuk navigasi dan penulisan
* *Head-tracking* dan *eye-tracking* untuk pengguna dengan mobilitas tangan sangat terbatas
:::

## Untuk kebutuhan kognitif dan belajar spesifik

::: {.incremental}
* Perangkat lunak *text-to-speech* dan *speech-to-text* untuk disleksia
* Aplikasi *mind-mapping* dan pengatur grafis (*graphic organizer*) untuk perencanaan tulisan
* Pengingat dan pengatur waktu visual untuk mendukung fungsi eksekutif (relevan untuk ADHD)
* Materi dengan bahasa yang disederhanakan (*plain language*) untuk gangguan pemrosesan bahasa
:::

# Peran Akal Imitasi dalam Aksesibilitas 🤖 {background-color="#14497F"}

## Yang bisa dibantu AI

::: {.incremental}
* **Transkripsi dan *live captioning* otomatis** — akurasi meningkat pesat, meski masih bermasalah untuk dialek/logat tertentu.
* **Deskripsi gambar otomatis (*alt text generation*)** — AI generatif mampu mendeskripsikan visual untuk pengguna *screen reader*.
* **Penyederhanaan teks** (*text simplification*) — meringkas atau menulis ulang bacaan kompleks ke tingkat keterbacaan yang lebih rendah.
* ***Augmentative and alternative communication* (AAC)** berbasis AI — memprediksi/menyusun kalimat bagi pengguna dengan hambatan bicara.
:::

## Risiko AI dalam aksesibilitas

::: {.incremental}
* **Bias pengenalan suara** — sistem *automated speech recognition* terbukti secara sistematis kurang akurat untuk penutur kulit hitam Amerika dibanding penutur kulit putih [(Koenecke et al., 2020)](https://doi.org/10.1073/pnas.1915768117).
* **Halusinasi pada deskripsi otomatis** — AI generatif bisa "mengarang" detail visual yang tidak ada, fatal bagi pengguna *screen reader* yang sepenuhnya bergantung pada AI.
* Kaitannya dengan kerangka ***automation* vs. *augmentation*** [(Fügener et al., 2026)](https://doi.org/10.1287/mnsc.2024.05684): AI sebaiknya **melengkapi** penilaian dan pendampingan manusia.
:::

# Prinsip Desain Aksesibel 📐 {background-color="#14497F"}

## WCAG: empat prinsip **POUR**

*Web Content Accessibility Guidelines* ([WCAG 2.1](https://www.w3.org/TR/WCAG21/)) merangkum aksesibilitas digital dalam 4 prinsip:

::: {.incremental}
* ***Perceivable*** — informasi harus dapat dipersepsikan lewat lebih dari satu indra (mis. teks alternatif untuk gambar).
* ***Operable*** — antarmuka harus bisa dioperasikan lewat berbagai cara input (mis. navigasi *keyboard* penuh).
* ***Understandable*** — informasi dan pengoperasian antarmuka harus mudah dipahami.
* ***Robust*** — konten harus kompatibel dengan berbagai teknologi pembantu, termasuk yang akan datang.
:::

::: aside
[World Wide Web Consortium (2018). *Web Content Accessibility Guidelines (WCAG) 2.1*](https://www.w3.org/TR/WCAG21/)
:::

## Menerapkannya di materi pembelajaran

::: {.incremental}
* Beri **alt-text*** pada semua gambar/diagram di *slide* dan modul.
* Sediakan **transkrip** untuk video/audio kuliah.
* Gunakan **kontras warna** yang cukup dan hindari mengandalkan warna semata untuk makna.
* Format dokumen (PDF/Word) dengan **struktur *heading*** yang benar agar terbaca *screen reader*.
* Berikan **fleksibilitas format tugas** sesuai prinsip UDL, tetapi bukan berarti menurunkan standar akademik.
:::

# Diskusi & Refleksi 💬 {background-color="#14497F"}

## Studi kasus untuk didiskusikan

::: {.callout-note}
### Skenario

Seorang peserta didik dengan disleksia berat meminta izin menggunakan *text-to-speech* dan *AI-assisted note-taking* selama pembelajaran dan ujian. Seorang peserta didik lain berpendapat ini "tidak adil" dibanding peserta didik yang mengerjakan tanpa bantuan AI apa pun.
:::

::: {.incremental}
* Apakah masalah ini berkaitan dengan **kesetaraan** (*equality*, semua diperlakukan sama) atau **keadilan** (*equity*, semua diberi apa yang dibutuhkan)?
* Bagaimana idealnya kebijakan kelas Anda **membedakan** antara teknologi pembantu yang bersifat **akomodasi** vs. yang berpotensi menjadi **kecurangan akademik**?
* Kaitkan dengan gagasan ***the effortless trap*** [(Brčić & Frljić, 2026)](https://doi.org/10.48550/arXiv.2606.26181). 
  - Kapan bantuan AI memberdayakan, dan kapan ia justru menggantikan proses belajar yang esensial?
:::

::: notes
Diskusi ±15 menit. Dorong mahasiswa membedakan disabilitas yang memengaruhi *akses* terhadap materi (aksesibilitas) vs. AI yang menggantikan *proses kognitif* itu sendiri (potensi "effortless trap"). Keduanya sering disalahpahami sebagai hal yang sama.
:::

## Rangkuman

::: {.incremental}
* Aksesibilitas adalah **hak** yang dijamin UU No. 19/2011 dan UU No. 8/2016.
* **Model sosial** membingkai disabilitas sebagai ketidaksesuaian individu-lingkungan, sehingga solusinya ada pada **desain**, bukan "perbaikan" individu.
* Empat kategori hambatan belajar (sensorik, motorik/fisik, kognitif, kesehatan mental) sering bersifat **situasional**, sehingga desain aksesibel menguntungkan semua orang (*curb-cut effect*).
* **UDL** (representasi, aksi & ekspresi, keterlibatan — jamak/*multiple means*) dirancang proaktif untuk semua peserta didik, bukan akomodasi *ad hoc* yang reaktif.
* **Teknologi pembantu** berjenjang dari rendah ke tinggi teknologi, dipilih sesuai kebutuhan spesifik tiap kategori hambatan.
* AI membantu aksesibilitas lewat transkripsi, deskripsi gambar otomatis, penyederhanaan teks, dan AAC — tapi membawa risiko bias pengenalan suara dan halusinasi deskripsi; perannya idealnya **augmentasi**, bukan pengganti penilaian manusia.
* **WCAG (POUR)** — *Perceivable, Operable, Understandable, Robust* — jadi prinsip praktis untuk merancang materi (alt-text, transkrip, kontras, struktur *heading*).
* Pertanyaan refleksi: dalam skenario tadi, di mana batas antara AI sebagai asisten (akomodasi) dan AI yang menggantikan proses belajar, dan siapa yang berhak menentukannya?
:::

## Terima kasih!😊 {.closing-slide}

::: {.closing-subtitle}
Ada pertanyaan?
:::

::: {.closing-contact}
* [ ]{.fa-solid .fa-envelope .fa-fw} [amelia.zein@psikologi.unair.ac.id](mailto:amelia.zein@psikologi.unair.ac.id)
* [ ]{.fa-solid .fa-globe .fa-fw} https://rameliaz.github.io/
* [ ]{.fa-brands .fa-discord .fa-fw} [@rameliaz](https://discord.com/users/807046335613632555)
* [ ]{.fa-brands .fa-keybase .fa-fw} [@rameliaz](https://keybase.io/rameliaz)
* [ ]{.fa-brands .fa-github .fa-fw} [@rameliaz](https://github.com/rameliaz)
:::

::: {.closing-note}
Paparan disusun dengan menggunakan {{< fa brands r-project >}} dan [Quarto](https://quarto.org/) dengan template dari [UNAIR Theme](https://github.com/rameliaz/quarto-unair-theme).
:::

## Rujukan {.scrollable .smaller}

* Brčić, M., & Frljić, S. (2026). *The effortless trap: Productive struggle, AI, and the illusion of learning*. arXiv. <https://doi.org/10.48550/arXiv.2606.26181>
* CAST. (2024). *Universal Design for Learning guidelines* (Version 2.2). <https://udlguidelines.cast.org/>
* Fügener, A., Walzner, D. D., & Gupta, A. (2026). Roles of artificial intelligence in collaboration with humans: Automation, augmentation, and the future of work. *Management Science, 72*(1), 538–557. <https://doi.org/10.1287/mnsc.2024.05684>
* Koenecke, A., Nam, A., Lake, E., Nudell, J., Quartey, M., Mengesha, Z., Toups, C., Rickford, J. R., Jurafsky, D., & Goel, S. (2020). Racial disparities in automated speech recognition. *Proceedings of the National Academy of Sciences, 117*(14), 7684–7689. <https://doi.org/10.1073/pnas.1915768117>
* U.S. Assistive Technology Act of 1998, 29 U.S.C. § 3002. <https://www.law.cornell.edu/uscode/text/29/3002>
* World Wide Web Consortium. (2018). *Web Content Accessibility Guidelines (WCAG) 2.1*. <https://www.w3.org/TR/WCAG21/>
